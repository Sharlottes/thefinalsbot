...
import { ActionRowBuilder, ApplicationCommandOptionType, ButtonBuilder, ButtonInteraction, ButtonStyle, ComponentType, EmbedBuilder, ModalBuilder, ModalSubmitInteraction, TextInputBuilder, TextInputStyle } from "discord.js";
...
export default class SearchLeaderboard {
    ...
    async search(
        ...
        const result = await fetch(`https://api.the-finals-leaderboard.com/v1/leaderboard/s2/crossplay?name=${name_tag}`)
    ...
    getLeaderBoardData(): LeaderBoardUserData {
        return this.leaderboard?.data?.[this.page] ||
            {
                rank: -1,
                change: 0,
                leagueNumber: 0,
    ...
    getEmbed(): EmbedBuilder {
        ...
        return new EmbedBuilder()
            ...
            .setAuthor({ name: `THE FINALS TEAMS`, iconURL: `https://cdn.discordapp.com/avatars/1219832567570890833/889af2fc8b96fc95cf833a4395092813.webp?size=1024` })
        ...
        for (let i = 0; i < btn_symbols.length; i++) {
            ...
            action_rows.addComponents(
                new ButtonBuilder()
                    ...
                    .setDisabled(this.page + btn_symbols[i].page < 0 || this.page + btn_symbols[i].page > ((this.leaderboard?.count || 0) - 1))
            );
        }
